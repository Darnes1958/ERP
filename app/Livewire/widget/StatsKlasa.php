    ...
    protected function getStats(): array
    {
        return [
          ...
          Stat::make('','')
            ...
            ->value(new HtmlString('<span class="text-primary-500">'.
              number_format(Receipt::where('receipt_date',$this->repDate)->where('imp_exp',0)->sum('val'),2, '.', ',').'</span>')),

          Stat::make('','')
            ->label(new HtmlString('<span class="text-indigo-700">دفع</span>'))
            ->value(new HtmlString('<span class="text-danger-600">'.
              number_format(Receipt::where('receipt_date',$this->repDate)->where('imp_exp',1)->sum('val'),2, '.', ',').'</span>')),
