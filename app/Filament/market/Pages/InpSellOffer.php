...
use App\Models\Sell_offer_tran_work;
use App\Models\Sell_offer_work;
...
class InpSellOffer extends Page implements HasSchemas,HasTable
{
    ...
    use Raseed;
    use PublicTrait;
    protected string $view = 'filament.market.pages.inp-sell-offer';
    ...
    {
        $this->sell = Sell_offer_work::find(auth()->id());
        if (!$this->sell) {
            if (Auth::user()->hasRole('admin'))
                $this->sell=Sell_offer_work::create([
    ...
    public function PrintOrder($id){
        ...
        $res=Sell_offer::find($id);
        $orderdetail=Sell_offer_tran::where('sell_id',$id)->get();


        $html = view('PDF.rep-order-sell-offer',
    ...
    {
        return $schema
            ->model(Sell_offer_tran_work::class)
                        ...
                        Select::make('item_id')
                            ...
                            ->suffixAction(
                                Action::make('select_item')
                                    ->label('')
                                    //->extraAttributes(['class'=>'text-xs text-blue-600',])
                                        ->color('info')
                    ...
                    ->schema([
                        Actions::make([
                            Action::make('store')
                                ...
                                ->button()
                                ->visible(function (){return Sell_offer_tran_work::where('sell_id',Auth::id())->count()>0;})
                                ->color('success')
                                ...
                                ->action(function () {
                                    ...
                                    $id=Sell_offer::create($this->sell->toArray());
                                    $selltran=Sell_offer_tran_work::where('sell_id',Auth::id())->get();
                                    foreach ($selltran as $tran) {
                                        $tran->sell_id=$id->id;
                                        unset($tran['id'],$tran['created_at'],$tran['updated_at']);
                                        Sell_offer_tran::create($tran->toArray());
                                    ...
                                    $this->sell=Sell_offer_work::find(Auth::id());
                                    $this->sell->tot=0; $this->sell->total=0;
                                    ...
                                    $this->sell->save();
                                    $this->sellForm->fill($this->sell->toArray());
                                    $this->selltran= Sell_offer_tran_work::where('sell_id', Auth::id())->delete();
                                    $this->sellTranForm->fill([]);
                            ...
                            Action::make('print')
                                ...
                                ->action(function (){
                                    ...
                                    $sell=Sell_offer::find($this->id_to_print);
                                    $res=Sell_offer_tran::where('sell_id',$this->id_to_print)->get();


                                    return Response::download(self::ret_spatie($res,
                                        'PDF.rep-order-sell-offer',['sell'=>$sell,]), 'filename.pdf', self::ret_spatie_header());
                                })
    ...
    public function tot(){
        $tot=Sell_offer_tran_work::where('sell_id',Auth::id())->sum('sub_tot');
        $this->sell->tot=$tot;
        $this->sell->differ=($tot+$this->sell->cost)*$this->sell->rate/100;
        $this->sell->total=$tot+$this->sell->cost+$this->sell->differ;
        $this->sell->save();
        $this->sellForm->fill($this->sell->toArray());
    }
    ...
    public function fill_item($item_id,$barcode){
        ...
        $this->selltran=Sell_offer_tran_work::where('sell_id',Auth::id())
    ...
    public function add_rec(){
        ...
        if ($this->selltran)
        {
            ...
            $this->selltran=Sell_offer_tran_work::
    ...
    public function table(Table $table):Table
    {
        return $table
            ->query(function () {
                $sell_tran = Sell_offer_tran_work::where('sell_id', Auth::id());
                return $sell_tran;
            })
            ->columns([
            ...
            ->recordActions([
                Action::make('delete')
                    ->action(function (Sell_offer_tran_work $record) {
                        $record->delete();
                        $this->tot();
                        $this->sellTranForm->fill([]);
                    })
